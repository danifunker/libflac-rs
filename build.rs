...
//!
//! The `cc` build-dependency and the entire oracle build live behind
//! `#[cfg(feature = "cref")]`, so a default build neither references nor resolves
//! `cc`: with the feature off there are zero dependencies of any kind.
...
mod cref {
    ...
    pub fn build_oracle() {
        ...
        // vendor dir is absent. Override either with FLAC_C_DIR (a libFLAC source
        // tree containing include/FLAC/ and src/libFLAC/).
        ...
        // support for the Ogg-FLAC differential tests. libFLAC delegates all Ogg
        // paging to libogg, so a byte-exact Ogg oracle needs it. Dev-only (cref/ is
        // excluded from the published crate, which stays pure-Rust).
        ...
        build
            ...
            // Mirror MAME's libFLAC define set (scripts/src/3rdparty.lua) so the
            // oracle matches chdman's macro environment exactly, then force the
            // *scalar* reference path with FLAC__NO_ASM (the `#ifndef FLAC__NO_ASM`
            // dispatch in stream_encoder.c is skipped, so no intrin symbols are
            // referenced even though config.h sets FLAC__HAS_X86INTRIN=1). config.h
            // supplies the CPU macros + PACKAGE_VERSION; release semantics via NDEBUG.
            //
            // FLAC__HAS_OGG=1 enables the Ogg path; it is fully runtime-gated on the
            // encoder's `is_ogg` flag, so native (non-Ogg) output is byte-identical
            // to a HAS_OGG=0 build — the existing differential tests confirm this.
        ...
        for f in [
            ...
            "stream_decoder.c",
            "bitreader.c",
            // Ogg layer: the encoder/decoder "aspects" that drive libogg, the
            // metadata rewrite helper, and the FLAC-in-Ogg mapping constants.
        ...
        // `libflac_rs_cref_encode` at test link time). So pass the archive straight
        // to the *test* linker, whole, so every object is pulled in unconditionally.
